	...
	*/
    $angular.module(APP_NAME).controller('VideoController',function($scope,$timeout,video,$http,$localStorage,$mdDialog,$interval) {
		...
		/*if (window.matchMedia("(orientation: portrait)").matches) {
			...
			document.getElementById('convideo').style.display = 'block';
			}
		...
		$.getJSON("button.json", function(result){
			...
			*/
			...
			$scope.ctime=function ctime(video)
			{
				...
				//if((window.matchMedia("(orientation: landscape)").matches)){
					...
					function countTimer() {
						...
						if((noq<=no_of_questions)&&(testend>waittime[jk])){
			...
			//} else if((window.matchMedia("(orientation: portrait)").matches)){alert("portrait"); clearInterval(progInc);}
		//});
	} 
